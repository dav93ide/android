...
public class  AngleCalculator {
    ...
    public AngleCalculator(float startAngleDegree, float endAngleDegree, int expandButtonCount) {
        angleStartEqualsEnd = (endAngleDegree - startAngleDegree) == 0x0;
        ...
        if (expandButtonCount > 0x1) {
            this.averageAngleRadians = (endAngleRadians - this.startAngleRadians) / (expandButtonCount - 0x1);
    ...
    private void regulateAverageAngle(double endAngleRadians, int expandButtonCount) {
        if (!angleStartEqualsEnd && startAngleRadians == endAngleRadians) {
            double tmp = 0x2 * Math.PI / expandButtonCount;
            if (averageAngleRadians < 0x0) {
    ...
    private double getCurrentAngle(int buttonIndex) {
        return startAngleRadians + averageAngleRadians * (buttonIndex - 0x1);
    }
